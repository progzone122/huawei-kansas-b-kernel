...
#ifdef CONFIG_COMPAT
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
static int exfat_generic_compat_ioctl(struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg);
#else
static long exfat_generic_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
#endif
#endif
...
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36)
static int exfat_generic_compat_ioctl(struct inode *inode, struct file *filp,
							   unsigned int cmd, unsigned long arg)
#else
static long exfat_generic_compat_ioctl(struct file *filp,
								unsigned int cmd, unsigned long arg)
#endif
{
#if !(LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36))
	...
	case EXFAT_IOC_GET_DEBUGFLAGS: {
		...
		flags = sbi->debug_flags;
		return put_user(flags, (int __user *)compat_ptr(arg));
	}
	case EXFAT_IOC_SET_DEBUGFLAGS: {
		struct super_block *sb = inode->i_sb;
		struct exfat_sb_info *sbi = EXFAT_SB(sb);
		...
		if (get_user(flags, (int __user *)compat_ptr(arg)))
			return -EFAULT;
