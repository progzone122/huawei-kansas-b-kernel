...
#ifdef CONFIG_HUAWEI_EMMC_DSM
	if (host != NULL)
		iomt_host_latency_mrq_end(host, mrq);
#endif
...
power_cycle:
	...
	{
		if(host && (!strcmp(mmc_hostname(host),"mmc1")))
			pr_err("%s: host and card voltage have changed into 1.8v success!\n", mmc_hostname(host));
	}
...
void mmc_rescan(struct work_struct *work)
{
	...
	if (host->rescan_disable)
	{
		if (!strcmp(mmc_hostname(host), "mmc1"))
		{
			pr_err("%s:%s:rescan_disable is true,so exit\n", mmc_hostname(host), __func__);
			pr_err("%s:mmc_rescan--\n", mmc_hostname(host));
		}
	...
	if (mmc_card_is_removable(host) && host->ops->get_cd &&
		...
		if (!strcmp(mmc_hostname(host), "mmc1"))
		{
			pr_err("%s:%s:host->ops->get_cd(host) == 0, maybe the sdcard is not present, so exit\n", mmc_hostname(host), __func__);
		}
		mmc_power_off(host);
		mmc_release_host(host);
		goto out;
		...
		if (!strcmp(mmc_hostname(host), "mmc1"))
			pr_err("%s:%d sdcard is present, prepare to init sdcard  \n", __func__, __LINE__);
	}
