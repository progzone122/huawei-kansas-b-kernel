...
void _ddic_test_read(void)
{
	...
	for (i = 0; i < 3; i++) {
		cmd_tab[i].dtype = 0X0A;
		cmd_tab[i].payload = vmalloc(4 * sizeof(unsigned char));
		memset(cmd_tab[i].payload, 0, 4);
	}

	do_lcm_vdo_lp_read(cmd_tab,3);

	for (i = 0; i < 3; i++)
		DDPMSG("read lcm addr:0x%x--val:0x%x\n",
		cmd_tab[i].dtype, *(unsigned int *)cmd_tab[i].payload);
...
void _ddic_test_read_write(void)
{
	...
	*(write_table1[2].payload) = 0x45;
	...
	for (i = 0; i < 3; i++)
		DDPMSG("read lcm addr:0x%x--val:0x%x\n",
			read_tab[i].dtype,
			*(unsigned int *)read_tab[i].payload);
