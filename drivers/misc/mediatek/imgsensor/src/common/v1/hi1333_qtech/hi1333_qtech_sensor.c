...
#define LOG_DBG(fmt, args...)    if (DEBUG_HI1333_QTECH) pr_debug(PFX "%s %d " fmt, __func__,__LINE__, ##args)
...
static void write_shutter(UINT32 shutter)
{
	...
	(void)imgsensor_sensor_i2c_write(&imgsensor, SENSOR_INTEG_TIME_HL, ((shutter & 0xFF0000) >> 16), IMGSENSOR_I2C_BYTE_DATA);
