...
#include "ov13855_ofilm_tdk_sensor.h"

#define RETRY_TIMES 2

/****************************Modify Following Strings for Debug****************************/
#define PFX "[ov13855_ofilm_tdk]"
#define DEBUG_OV13855_OFILM_TDK 0
#define LOG_DBG(fmt, args...)    if (DEBUG_OV13855_OFILM_TDK) pr_debug(PFX "%s %d " fmt, __func__,__LINE__, ##args)
...
static void set_dummy(void)
{
    ...
    rc = imgsensor_sensor_i2c_write(&imgsensor, SENSOR_TIMEING_VTS_REG_H,
    ...
    rc = imgsensor_sensor_i2c_write(&imgsensor,SENSOR_TIMEING_HTS_REG_H,
...
/*************************************************************************
* FUNCTION
*   calc_long_expo_reg_value
*
* DESCRIPTION
*   This function calc long expo reg value.
*
* PARAMETERS
*   System Clock (Mhz): imgsensor_info.cap.pclk/1000000
	LONG_EXPO_HTS:HTS, (HEX, R380c:R380d), imgsensor_info.cap.linelength
*	Exp:(ms, R3502-R3500)
*	tline:HTS/System Clock
*	Long Exposure time:shutter*tline
*	rest exp. Time (ms):(Long Exposure time-exp)
* RETURNS
*   reg value
*
* GLOBALS AFFECTED
...
static kal_uint32 calc_long_expo_reg_value(UINT32 shutter)
{
	...
	unsigned long long  exp = ((LONG_EXPO_REG_H_VALUE*256*256 + LONG_EXPO_REG_M_VALUE*256 + LONG_EXPO_REG_L_VALUE)
	*imgsensor_info.cap.linelength)/(16*imgsensor_info.cap.pclk/1000000); //us
	unsigned long long rest_exp_time = (shutter * imgsensor_info.cap.linelength)/(imgsensor_info.cap.pclk/1000000) - exp ; //us
	reg_value = (rest_exp_time*(imgsensor_info.cap.pclk/1000000))/(256);
	return reg_value;
}
...
static void set_shutter(UINT32 shutter)
{
	...
	if (imgsensor.autoflicker_en) {
		...
		if(realtime_fps >= 298 && realtime_fps <= 305){
			...
			(void)imgsensor_sensor_i2c_write(&imgsensor, SENSOR_TIMEING_VTS_REG_H,
					imgsensor.frame_length & 0xFFFF, IMGSENSOR_I2C_WORD_DATA);
		}
	} else {
		/*3) otherwise, update frame length directly*/
	...
	long_expo_time = (shutter*imgsensor_info.cap.linelength)/(imgsensor_info.cap.pclk/1000000);
	if(long_expo_time <= NORMAL_EXPOSE_TIME_MAX)
	{
		(void)imgsensor_sensor_i2c_write(&imgsensor, SENSOR_LONG_EXPO_REG_H,
				(shutter >> 12) & 0x0F, IMGSENSOR_I2C_BYTE_DATA);
		(void)imgsensor_sensor_i2c_write(&imgsensor, SENSOR_LONG_EXPO_REG_M,
				(shutter >> 4 ) & 0xFF, IMGSENSOR_I2C_BYTE_DATA);
		(void)imgsensor_sensor_i2c_write(&imgsensor, SENSOR_LONG_EXPO_REG_L,
				(shutter << 4)  & 0xF0, IMGSENSOR_I2C_BYTE_DATA);
	}
	else //If the exposure time is greater than 0.3s,write  the register of 0x3c9a-0x3c9d
	{
		tmp_reg_value =  calc_long_expo_reg_value(shutter);
		LOG_INF("long time expo reg value is 0x%x\n",tmp_reg_value);
		imgsensor_sensor_write_setting(&imgsensor, &tdk_normal_to_long_ready_settings);
		(void)imgsensor_sensor_i2c_write(&imgsensor, 0x3c9a,
			((tmp_reg_value>>16)&0xFFFF), IMGSENSOR_I2C_WORD_DATA);
		(void)imgsensor_sensor_i2c_write(&imgsensor, 0x3c9c,
			(tmp_reg_value&0xFFFF), IMGSENSOR_I2C_WORD_DATA);
		imgsensor_sensor_write_setting(&imgsensor, &tdk_normal_to_long_end_settings);
	}
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	iReg = gain*REG_GAIN_1X / BASEGAIN;
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	(void)imgsensor_sensor_i2c_write(&imgsensor, SENSOR_LONG_GAIN_REG_H,
...
static kal_uint32 feature_control_ov13855_ofilm_tdk(MSDK_SENSOR_FEATURE_ENUM feature_id,
					UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_GET_PDAF_INFO:
		...
		PDAFinfo= (struct SET_PD_BLOCK_INFO_T *)(uintptr_t)(*(feature_data+1)); // copy
		switch( *feature_data){
		case MSDK_SCENARIO_ID_CAMERA_CAPTURE_JPEG:
		case MSDK_SCENARIO_ID_VIDEO_PREVIEW:
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	get_info,
	get_resolution,
	feature_control_ov13855_ofilm_tdk,
	control,
	close
};


UINT32 OV13855_OFILM_TDK_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}   /*  OV113855_MIPI_RAW_SensorInit  */
