...
#include "imx258_sunny_zet_sensor.h"

static DEFINE_SPINLOCK(imgsensor_drv_lock);

/****************************Modify Following Strings for Debug****************************/
#define PFX "[imx258_sunny_zet]"
#define DEBUG_IMX258_SUNNY_ZET 0
#define LOG_DBG(fmt, args...)    if (DEBUG_IMX258_SUNNY_ZET) pr_debug(PFX "%s %d " fmt, __func__,__LINE__, ##args)
	...
	if(imgsensor_sensor_i2c_write(&imgsensor, SENSOR_SENSOR_CTRL_REG_L,0x0F, IMGSENSOR_I2C_BYTE_DATA) == 0)
	{
		...
		if(tmp_reg_value == 0x01)
		{
			...
			if( rc < 0 ){
				...
				imx258_type = IMX258_SUNNY_ZET_NOPDAF_TYPE;
			}
			if(imx258_type != IMX258_SUNNY_ZET_HDD_TYPE)
			{
				imx258_type = IMX258_SUNNY_ZET_NOPDAF_TYPE;
			}
...
static kal_uint16 gain2reg(const kal_uint16 gain)
{
	...
	for (i = 0; i < IMX258MIPI_MaxGainIndex; i++) {
		if (gain <= IMX258MIPI_zet_sensorGainMapping[i][0])
			break;
	}
	...
	if (gain != IMX258MIPI_zet_sensorGainMapping[i][0])
		LOG_DBG("Gain mapping don't correctly:%d %d\n", gain,
			IMX258MIPI_zet_sensorGainMapping[i][0]);
	return IMX258MIPI_zet_sensorGainMapping[i][1];
}
...
static kal_uint32 feature_control_imx258_sunny_zet(MSDK_SENSOR_FEATURE_ENUM feature_id,
					UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	case SENSOR_FEATURE_SET_PDAF_TYPE:
		imx258_type = IMX258_SUNNY_ZET_HDD_TYPE;
		break;
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	get_info,
	get_resolution,
	feature_control_imx258_sunny_zet,
	control,
	close
};

UINT32 IMX258_SUNNY_ZET_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
