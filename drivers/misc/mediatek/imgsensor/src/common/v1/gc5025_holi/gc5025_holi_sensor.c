...
#define DEBUG_GC5025_HOLI 0

#define PFX "GC5025_HOLI"
#define LOG_INF(fmt, args...)   if (DEBUG_GC5025_HOLI) pr_err(PFX "[%s] " fmt, __FUNCTION__, ##args)
...
static struct GC5025_HOLI_OTP gc5025_otp_info;
...
static kal_uint16 gc5025_gcore_calc_ddparam(kal_uint16 size, kal_uint16 *buff)
{
	...
	for (i = 0; i < total_number; i++) {
		LOG_INF("GC5025_OTP_DD:buff[%d] = (0x%x, 0x%x, 0x%x, 0x%x)\n",
...
static kal_uint32 feature_control_gc5025_holi(MSDK_SENSOR_FEATURE_ENUM feature_id,
...
UINT32 GC5025_HOLI_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
