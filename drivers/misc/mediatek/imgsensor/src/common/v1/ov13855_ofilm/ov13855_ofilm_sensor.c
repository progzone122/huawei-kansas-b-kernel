...
#define LOG_DBG(fmt, args...)    if (DEBUG_OV13855_OFILM) pr_debug(PFX "%s %d " fmt, __func__,__LINE__, ##args)
...
static void set_shutter(UINT32 shutter)
{
	...
	else //If the exposure time is greater than 0.3s,write  the register of 0x3c9a-0x3c9d
	{
		...
		imgsensor_sensor_write_setting(&imgsensor, &normal_to_long_ready_settings);
		...
		imgsensor_sensor_write_setting(&imgsensor, &normal_to_long_end_settings);
	}
...
UINT32 OV13855_OFILM_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
