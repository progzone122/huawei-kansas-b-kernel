...
/*#define IMGSENSOR_DT_DEBUG_ENABLE*/
...
static int imgsensor_dt_string_to_enum( const char *string, const struct IMGSENSOR_STRING_TO_ENUM *map, unsigned int map_size)
{
	unsigned int i = 0;
...
static int imgsensor_dt_get_power_info(struct device_node  *of_node, int index,
	struct IMGSENSOR_HW_CUSTOM_POWER_INFO *pwr_info, unsigned int max_size)
{
	int rc = 0, i = 0;

	const char *id_name = NULL;
	const char *pin_name = NULL;
	int id_count = 0, pin_count = 0;
	...
	snprintf(property0, IMGSENSOR_PROPERTY_MAXSIZE, "hw_cfg,power-type%d", index);
	...
	snprintf(property1, IMGSENSOR_PROPERTY_MAXSIZE, "hw_cfg,power-pin%d", index);
	...
	for (i = 0; i < id_count; i++){
		/*1st get id*/
		rc = of_property_read_string_index(of_node, property0, i, &id_name);
		if( rc < 0 ){
		...
		/*2nd get pin*/
		rc = of_property_read_string_index(of_node, property1, i, &pin_name);
		if( rc < 0 ){
			pr_err("%d: get the %dth %s property failed\n",
...
void imgsensor_dt_pwr_config_debug(struct IMGSENSOR_HW_CFG *pwr_cfg, unsigned int pwr_cfg_size)
{
	int i = 0, j = 0;
...
int imgsensor_dt_hw_config_init(struct IMGSENSOR_HW_CFG *pwr_cfg, unsigned int pwr_cfg_size)
{
	...
	for(i=0; i<sensor_num; i++){
		snprintf(property, IMGSENSOR_PROPERTY_MAXSIZE, "hw_cfg,sensor-idx%d", i);
		rc = of_property_read_u32(of_node, property, &pwr_cfg[i].sensor_idx);
		if (rc < 0) {
			pr_err("%d failed rc %d\n", __LINE__,rc);
			return rc;
		}
		pr_info("%s %d: %s pwr_cfg[%d].sensor_idx=%d\n",
			__func__, __LINE__, property, i,pwr_cfg[i].sensor_idx);

		snprintf(property, IMGSENSOR_PROPERTY_MAXSIZE, "hw_cfg,i2c-dev%d", i);
		rc = of_property_read_u32(of_node, property, &pwr_cfg[i].i2c_dev);
		if (rc < 0) {
			pr_err("%d failed rc %d\n", __LINE__, rc);
			return rc;
		}
		...
		rc = imgsensor_dt_get_power_info(of_node, i,
			pwr_cfg[i].pwr_info, IMGSENSOR_HW_POWER_INFO_MAX);
		if( rc < 0 ){
