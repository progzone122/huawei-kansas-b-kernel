...
#define DEBUG_HI846_TRULY 0
#define LOG_DBG(fmt, args...)    if (DEBUG_HI846_TRULY) pr_debug(PFX "%s %d " fmt, __func__,__LINE__, ##args)
...
static void set_dummy(void)
{
	...
	(void)imgsensor_sensor_i2c_write(&imgsensor, SENSOR_LINE_LENGTH_PCK_REG_H, imgsensor.line_length, IMGSENSOR_I2C_WORD_DATA);
}	/*	set_dummy  */
	...
	if(feature_para != NULL && feature_para_len != NULL){
		...
		switch (feature_id) {
			...
			case SENSOR_FEATURE_GET_DEFAULT_FRAME_RATE_BY_SCENARIO:
				get_default_framerate_by_scenario((enum MSDK_SCENARIO_ID_ENUM)*(feature_data), (UINT32 *)(uintptr_t)(*(feature_data+1)));
				break;
...
UINT32 HI846_TRULY_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
