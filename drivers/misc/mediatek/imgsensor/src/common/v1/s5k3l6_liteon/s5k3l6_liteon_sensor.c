...
#include "s5k3l6_liteon_sensor.h"

#define RETRY_TIMES 2

/****************************Modify Following Strings for Debug****************************/
#define PFX "[s5k3l6_liteon]"
#define DEBUG_S5K3L6_LITEON 0
#define LOG_DBG(fmt, args...)    if (DEBUG_S5K3L6_LITEON) pr_debug(PFX "%s %d " fmt, __func__,__LINE__, ##args)
...
static void set_normal_shutter( kal_uint32 shutter)
{
	...
	if (imgsensor.autoflicker_en) {
		...
		if(realtime_fps >= 298 && realtime_fps <= 305){
			/* 1) calc fps between 297~305, real fps set to 298*/
...
static imgsensor_shutter_mode_t get_shutter_mode( kal_uint32 in_shutter, kal_uint32 *out_shutter )
{
	kal_uint32 normal_max_shutter = 0;
	kal_uint32 cur_shutter = 0; //shutter needed to set in current(Nth) frame.
	static kal_uint32 pre_shutter = 0; //shutter be set in previous((N-1)th) frame.
	static kal_uint32 last_shutter = 0;//store the last shutter set by hal.
	imgsensor_shutter_mode_t shutter_mode = SHUTTER_NORMAL;

	if ( !out_shutter ){
		LOG_ERR("Fatal err, out_shutter is NULL\n");
		return SHUTTER_INVALID;
	}
	if( imgsensor_info.max_frame_length > imgsensor_info.margin ){
		normal_max_shutter = imgsensor_info.max_frame_length - imgsensor_info.margin;
	} else {
	...
	if ( in_shutter > normal_max_shutter && last_shutter <= normal_max_shutter ){
	...
	} else if ( in_shutter <= normal_max_shutter && last_shutter > normal_max_shutter ){
	...
	/*2. update shutter_mode base on cur_shutter and previous shutter which set to the sensor*/
	if( cur_shutter > normal_max_shutter && pre_shutter <= normal_max_shutter){
		/*the shutter to be set this time is long, but last is normal, NORMAL2LONG*/
		cur_shutter = last_shutter;
		shutter_mode = SHUTTER_NORMAL2LONG;
	} else if ( cur_shutter <= normal_max_shutter && pre_shutter > normal_max_shutter ){
		/*the shutter to set this time is normal, but last is long, LONG2NORMAL*/
		cur_shutter = last_shutter;
		shutter_mode = SHUTTER_LONG2NORMAL;
	} else if (cur_shutter > normal_max_shutter && pre_shutter > normal_max_shutter ){
		/*the shutter to be set this time is long, and last is long too, LONG*/
		shutter_mode = SHUTTER_LONG;
	} else {
		/*the shutter to be set this time is normal, and last is normal too, NORMAL*/
...
static void set_shutter(kal_uint32 shutter)
{
	...
	imgsensor_shutter_mode_t shutter_mode;
	...
	switch( shutter_mode ){
		...
		case SHUTTER_NORMAL2LONG:
			LOG_INF("shutter:%d.\n", cur_shutter);
			cur_shutter = cur_shutter * imgsensor.line_length / SENSOR_LONG_SHUTTER_LINELENGTH;
			cur_shutter = (cur_shutter < imgsensor_info.min_shutter) ? imgsensor_info.min_shutter : cur_shutter;
			cur_shutter = (cur_shutter > (imgsensor_info.max_frame_length - imgsensor_info.margin)) ?\
				(imgsensor_info.max_frame_length - imgsensor_info.margin) : cur_shutter;
			...
			LOG_INF("normal2long: shutter_val:%d, fll_val:%d.\n", cur_shutter, cur_frame_length);

			(void)streaming_control(KAL_FALSE);
			/*change to longshutter mode*/
			(void)imgsensor_sensor_write_setting(&imgsensor, &normal2long_setting_array);
			(void)imgsensor_sensor_i2c_write(&imgsensor, SENSOR_FRM_LENGHT_LINE_REG_H,
					cur_frame_length & 0xFFFF, IMGSENSOR_I2C_WORD_DATA);
			(void)imgsensor_sensor_i2c_write(&imgsensor, SENSOR_COARSE_INT_TIME_REG_H,
					cur_shutter & 0xFFFF, IMGSENSOR_I2C_WORD_DATA);
			/*stream on*/
		...
		case SHUTTER_LONG2NORMAL:
			...
			(void)streaming_control(KAL_FALSE);
			(void)imgsensor_sensor_write_setting(&imgsensor, &long2normal_setting_array);
			(void)imgsensor_sensor_i2c_write(&imgsensor, SENSOR_FRM_LENGHT_LINE_REG_H,
				cur_frame_length & 0xFFFF, IMGSENSOR_I2C_WORD_DATA);
			(void)imgsensor_sensor_i2c_write(&imgsensor, SENSOR_COARSE_INT_TIME_REG_H,
					cur_shutter & 0xFFFF, IMGSENSOR_I2C_WORD_DATA);
...
static kal_uint32 feature_control_s5k3l6_liteon(MSDK_SENSOR_FEATURE_ENUM feature_id,
...
static struct SENSOR_FUNCTION_STRUCT sensor_func = {
	...
	get_info,
	get_resolution,
	feature_control_s5k3l6_liteon,
	control,
	close
};


UINT32 S5K3L6_LITEON_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
