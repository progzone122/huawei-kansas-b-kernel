...
#ifdef HANDLE_CAM_DDR
static struct pm_qos_request ddr_request;
#endif
void mmdvfs_set_camera_on(bool is_camera_on)
{
#ifdef HANDLE_CAM_DDR
	if (is_camera_on)
		pm_qos_update_request(&ddr_request, 0);
	else
		pm_qos_update_request(&ddr_request,
			PM_QOS_DDR_OPP_DEFAULT_VALUE);
#endif
	pr_notice("set is_camera_on=%d\n", is_camera_on);
}
EXPORT_SYMBOL_GPL(mmdvfs_set_camera_on);
