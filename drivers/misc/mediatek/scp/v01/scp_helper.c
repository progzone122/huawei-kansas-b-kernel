 ...
 *        -1 :get sema timeout
 */
int get_scp_semaphore(unsigned int flag)
{
	int read_back;
	int count = 0;
	int ret = -1;
 ...
 *        -1 :release sema fail
 */
int release_scp_semaphore(unsigned int flag)
{
	int read_back;
	int ret = -1;
	unsigned long spin_flags;
...
int sensor_params_to_scp(u64 addr_vir, size_t size)
{
	int *params;

	params = (int *)addr_vir;
	sensor_para = (struct sensor_para_t *)params;
	sensor_para->acc_para.axis_map_x = 0;
	sensor_para->acc_para.axis_map_y = 2; //here for test share dram
	sensor_para->acc_para.axis_map_z = 1; //here for test share dram
	//here add all sensor para
	return 0;
}
...
+ *
+ * Note: The function should be called before disabling 26M & reseting scp.
+ */
static int params_to_scp(void)
{
	u64 addr4param = (u64)(SCP_TCM + SCP_REGION_INFO_OFFSET + OFF_PARAM_START);
	int ret = 0;

	mt_reg_sync_writel(scp_get_reserve_mem_phys(SCP_DRV_PARAMS_MEM_ID),
			addr4param);
	if (sensor_params_to_scp) {
		ret = sensor_params_to_scp(scp_get_reserve_mem_virt(SCP_DRV_PARAMS_MEM_ID),
 ...
 * 1. disabling 26M, 2. reseting SCP
 */
	if (params_to_scp() != 0)
        goto err;
