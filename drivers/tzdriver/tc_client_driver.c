...
static int calc_process_path_hash(unsigned char *data, unsigned long len, char *digest)
{
	...
	desc = kmalloc(sizeof(struct shash_desc)
			+ TZ_CRYPTO_SHASH_DESCSIZE, GFP_KERNEL);
#else
	desc = kmalloc(sizeof(struct shash_desc)
...
static int tee_calc_task_hash(unsigned char *digest, bool cfc_rehash, struct task_struct *S)
{
	...
	struct {
		struct shash_desc shash;
#ifdef CONFIG_HISI_CLANG
		char ctx[TZ_CRYPTO_SHASH_DESCSIZE];
#else
...
#else
		ret2 =
			close_session(dev_file, context->uuid, context->session_id);
#endif
	...
	for (index = 0; index < load_times;	index++) {
		...
		*(int *)p = load_flag;
		...
		if (ret == 0 && load_flag == 0) {
			...
			tloge("need to load configid =%d,uuid=%x\n", configid, uuidReturn->timeLow);
			if ( checkresult) {
