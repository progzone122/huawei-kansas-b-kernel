...
/* un-implement APIs on mtk */
static inline int hisi_secs_power_on(void)
{
	return 0;
}

static inline int hisi_secs_power_down(void)
{
	return 0;
}

static inline int spi_init_secos(unsigned int spi_bus_id)
{
	return 0;
}

static inline int spi_exit_secos(unsigned int spi_bus_id)
{
	return 0;
}
...
{
	u64 x0 = cmd;
	u64 x1 = SMC_OPS_NORMAL;
	u64 x2 = cmd_type;
	u64 x3 = cmd_addr;
	u64 x4 = 0;
	u64 ret, exit_reason = 0, ta, target;
...
retry:
	...
	if (exit_reason == SMC_EXIT_PREEMPTED && wait)
		goto retry;
