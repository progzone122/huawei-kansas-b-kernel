...
u32 lcd_kit_get_blmaxnit(struct mtk_panel_info* pinfo)
{
	...
	lcd_kit_brightness_ddic_info = pinfo->blmaxnit.lcd_kit_brightness_ddic_info;
	if (pinfo->blmaxnit.get_blmaxnit_type == GET_BLMAXNIT_FROM_DDIC && lcd_kit_brightness_ddic_info > BL_MIN && lcd_kit_brightness_ddic_info < BL_MAX){
...
static int lcd_kit_pinfo_init(char* compatible, struct mtk_panel_info* pinfo)
{
	...
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-pxl-clk", &pinfo->pxl_clk_rate, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-pxl-clk-div", &pinfo->pxl_clk_rate_div, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-data-rate", &pinfo->data_rate, 0);
	...
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-lcm-type", &pinfo->panel_lcm_type, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-ldi-dsi-mode", &pinfo->panel_dsi_mode, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-dsi-switch-mode", &pinfo->panel_dsi_switch_mode, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-ldi-trans-seq", &pinfo->panel_trans_seq, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-ldi-data-padding", &pinfo->panel_data_padding, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-ldi-packet-size", &pinfo->panel_packtet_size, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-ldi-ps", &pinfo->panel_ps, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-vfp-lp", &pinfo->ldi.v_front_porch_forlp, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-fbk-div", &pinfo->pxl_fbk_div, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,ssc-disable", &pinfo->ssc_disable, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-bl-ic-ctrl-type", &pinfo->bl_ic_ctrl_mode, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-bias-ic-ctrl-type", &pinfo->bias_ic_ctrl_mode, 0);

	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-bl-max-nit", &pinfo->bl_max_nit, 0);
	lcd_kit_parse_get_u32_default(compatible, "lcd-kit,panel-getblmaxnit-type", &pinfo->blmaxnit.get_blmaxnit_type, 0);
	if(pinfo->blmaxnit.get_blmaxnit_type == GET_BLMAXNIT_FROM_DDIC)
	{
		lcd_kit_parse_dcs_cmds(compatible, "lcd-kit,panel-bl-maxnit-command", "lcd-kit,panel-bl-maxnit-command-state", &pinfo->blmaxnit.bl_maxnit_cmds);
	}
	return ret;
}
