...
int lcd_kit_get_panel_id_info(struct hisi_fb_data_type* hisifd)
{
	...
	ret = lcd_kit_dsi_cmds_rx(hisifd, read_value, &disp_info->brightness_color_uniform.modulesn_cmds);
	...
	lcd_kit_dsi_cmds_rx(hisifd, read_value, &disp_info->brightness_color_uniform.equipid_cmds);
	lcd_panelid.equipid =  (uint32_t)read_value[0];
	memset(read_value,0,MAX_READ_SIZE);
	lcd_kit_dsi_cmds_rx(hisifd, read_value, &disp_info->brightness_color_uniform.modulemanufact_cmds);
