...
{
	struct cs35l41_coeff_desc *coeff_desc;
...
static int low_temperature_state_get(void)
{
	...
	temp = get_loginfo_int(batt_psy, POWER_SUPPLY_PROP_TEMP)/10;
...
static int cs35l41_spk_power_down(struct cs35l41_private *cs35l41)
{
	...
	// max delay is 100ms for power down
	for (i = 0; i < 100; i++) {
	...
	regmap_register_patch(cs35l41->regmap, cs35l41_pdn_patch,
...
/*
	if (!cs35l41->diag_en)
		request_firmware_nowait(THIS_MODULE, FW_ACTION_HOTPLUG,
			CS35L41_COEFF0_NAME[GV_BOX_NUM][CONFIG_NUM-2], dev, GFP_KERNEL,
			cs35l41, cs35l41_coeff_file_load);
*/
					...
					const char *filename)
{
	...
	const struct firmware *coeff_file;
...
static int cs35l41_otp_unpack(void *data)
{
	...
	if (cs35l41->bus_spi) {
		...
		dev_dbg(cs35l41->dev, "spi speed is %d\n",  spi->max_speed_hz);
		#if 0
...
//this is used for testing regmap_bulk_read() if needed
#if 0
	ret = regmap_bulk_read(cs35l41->regmap, CS35L41_OTP_MEM0, otp_mem,
						CS35L41_OTP_SIZE_WORDS);
	dev_info(cs35l41->dev, "Read otp_mem (base address 0x%x) with ret = %d\n",
			CS35L41_OTP_MEM0, ret);
	for (i=0; i<CS35L41_OTP_SIZE_WORDS; i++) {
		dev_info(cs35l41->dev, "regmap_bulk_read : otp_mem[%d] = 0x%08x\n", i, otp_mem[i]);
	}
	if (ret < 0) {
		dev_err(cs35l41->dev, "Read OTP Mem failed\n");
		return -EINVAL;
	}
	for (i = 0; i < ARRAY_SIZE(otp_mem); i++) {
		regmap_read(cs35l41->regmap, CS35L41_OTP_MEM0 + 4 * i,
			otp_mem + i);
		dev_info(cs35l41->dev, "regmap_read : otp_mem[%d] = 0x%08x\n", i, otp_mem[i]);

	}
	/*
	* The following is just for test in this patch.
	* Values are still applied from the OTP space starting at CS35L41_OTP_MEM0
	*/
	ret = regmap_bulk_read(cs35l41->regmap,
				CS35L41_OTP_CTRL0,
				otp_ctrl_mem,
				9);
	dev_info(cs35l41->dev, "Read OTP ctrl (base address 0x%x) with ret = %d\n",
		CS35L41_OTP_CTRL0, ret);
	for (i=0; i<9; i++) {
		dev_info(cs35l41->dev, "regmap_bulk_read: otp_ctrl_mem[%d] = 0x%08x\n", i, otp_ctrl_mem[i]);
	}
	for (i = 0; i < ARRAY_SIZE(otp_ctrl_mem); i++) {
		regmap_read(cs35l41->regmap, CS35L41_OTP_CTRL0 + 4 * i,
		otp_ctrl_mem + i);
		dev_info(cs35l41->dev, "regmap_read: otp_ctrl_mem[%d] = 0x%08x\n", i, otp_ctrl_mem[i]);

	}

	ret = regmap_bulk_read(cs35l41->regmap,
				CS35L41_DSP1_YMEM_UNPACK32_0,
				ymem_log,
				CS35L41_OTP_SIZE_WORDS);
	dev_info(cs35l41->dev, "Read YMEM (base address 0x%x) with ret = %d\n",
		CS35L41_DSP1_YMEM_UNPACK32_0, ret);
	for (i=0; i<CS35L41_OTP_SIZE_WORDS; i++) {
		dev_info(cs35l41->dev, "regmap_bulk_read: ymem_log[%d] = 0x%08x\n", i, ymem_log[i]);
	}
#endif
	...
	for (i = 0; i < otp_map_match->num_elements; i++) {
		...
		//dev_dbg(cs35l41->dev, "ymem_log[%d] =  Ox%08x, otp_val = 0x%x\n", word_offset,
		//			ymem_log[word_offset], otp_val);
		if (otp_map[i].reg != 0) {
			retry = 3;
			while(retry--) {
				 ...
				 void __user *arg)
{
	...
	struct cs35l41_cal_data cal_data;
	char *config;
	const struct firmware *fw;
	...
	case CS35L41_SPK_POWER_ON:
		dev_info(cs35l41->dev, "cs35l41 power on");
#ifdef ESD_TEST
		regcache_cache_bypass(cs35l41->regmap, true);
		regmap_read(cs35l41->regmap, CS35L41_ESD_STAGE, &val);
		regcache_cache_bypass(cs35l41->regmap, false);
	...
	case CS35L41_SPK_POWER_OFF:
		dev_info(cs35l41->dev, "cs35l41 power off");
	...
	case CS35L41_SPK_SWITCH_CALIBRATION:
		dev_info(cs35l41->dev, "cs35l41 calib");
		box = val / 100 -1;
	...
	case CS35L41_SPK_SWITCH_CONFIGURATION:
		dev_info(cs35l41->dev, "cs35l41 set tuning, val = %d", val);
	...
	case CS35L41_SPK_SET_R0:
		dev_info(cs35l41->dev, "CS35L41_SPK_SET_R0, value = 0x%x\n", val);
		if (val) {
			cal_data.status = 1;
...
static int cs35l41_init(struct cs35l41_private *cs35l41)
{
	...
	*/
	...
	gpio_irq = of_get_named_gpio(cs35l41->dev->of_node, "irq-gpios", 0);

	if (gpio_is_valid(gpio_irq)) {
		ret = gpio_request(gpio_irq, "cs35l41_irq");
		if (ret < 0) {
...
skip_irq_config:
	...
	dev_info(cs35l41->dev, "%s: CS35L41_IRQ1_MASK1 0x%x \n", __func__, int_status);
	/* unmask reference clk misssing interrupt */
	regmap_write(cs35l41->regmap, CS35L41_IRQ1_MASK3,
			0xffff867f);
	regmap_read(cs35l41->regmap, CS35L41_IRQ1_MASK3, &int_status);
	dev_info(cs35l41->dev, "%s: CS35L41_IRQ1_MASK3 0x%x \n", __func__, int_status);
...
static int cs35l41_recovery(struct cs35l41_private *cs35l41)
{
	...
	if (ret < 0) {
			dev_err(cs35l41->dev, "Failed to initialize CS35L41 amplifier: %d\n", ret);
			return ret;
	}
	...
	if (ret == 0) {
		if ((val & CS35L41_ESD_STAGE_FINAL_MASK) == CS35L41_ESD_STAGE_FINAL)
			dev_err(cs35l41->dev, "CS35L41 RECOVERY SUCCESS\n");
		else {
			dev_err(cs35l41->dev,
...
static int cs35l41_spi_probe(struct spi_device *spi)
{
	...
	int ret;
	u32 i;
	printk(KERN_ERR "cs35l41_spi_probe begin");
	...
	dev_info(&spi->dev, "set mtk spi clk!!");
	...
	cs35l41->reset_gpio = of_get_named_gpio(cs35l41->dev->of_node, "reset-gpios", 0);
	  if (!gpio_is_valid(cs35l41->reset_gpio)) {
				   ...
				   ret = gpio_request(cs35l41->reset_gpio, "cs35l41 reset");
				   if (ret) {
...
#define module_driver_cs(__driver, __register, __unregister, ...) \
...
#define module_spi_driver_cs(__spi_driver) \
        module_driver_cs(__spi_driver, spi_register_driver, \
                        spi_unregister_driver)

module_spi_driver_cs(cs35l41_spi_driver);

MODULE_DESCRIPTION("Misc CS35L41 driver");
