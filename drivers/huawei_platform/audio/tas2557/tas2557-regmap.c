...
#ifdef ENABLE_TILOAD
struct mutex tiload_opened_lock;
#include "tiload.h"
#endif

#define LOW_TEMPERATURE_GAIN 6
#define LOW_TEMPERATURE_COUNTER 12
char * boxtable[]={"DEFAULT", "AAC", "GK", "GD", "LC", "LX", "XW"};
char tas2557_fw_name[MAX_FW_NAME];
...
#ifdef CONFIG_HUAWEI_DSM_AUDIO
			audio_dsm_report_info(AUDIO_SMARTPA, DSM_SMARTPA_INT_ERR, "DEVA clock lost nDevInt1Status:%x nDevInt2Status:%x\n", nDevInt1Status, nDevInt2Status);
#endif
...
*/
int tas2557_i2c_probe(struct i2c_client *pClient,
	const struct i2c_device_id *pID)
{
	...
	if (nResult < 0) {
		...
		audio_dsm_report_info(AUDIO_SMARTPA, DSM_SMARTPA_I2C_ERR, "I2C error, Result:%d, Function:%s, Line:%d\n", nResult, __func__, __LINE__);
#endif
		dev_err(&pClient->dev, "I2C error, %d\n", nResult);
		goto err;
	}
    ...
    pr_err("tas2557 prode end");
err:

	return nResult;
}

EXPORT_SYMBOL(tas2557_i2c_probe);
...
#ifdef ENABLE_TILOAD
	mutex_destroy(&tiload_opened_lock);
#endif

	mutex_destroy(&pTAS2557->dev_lock);
	return 0;
}
EXPORT_SYMBOL(tas2557_i2c_remove);
