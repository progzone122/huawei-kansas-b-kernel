...
static char battery_vendor_name[50]="NONSTANDARD";

#define MAH_TO_UAH    100
#define MAX_DESIGN_UAH 3000000
#define MIN_DESIGN_UAH 1500000
#define EXITLOOP	 36500
#define FCC_MAX_DESIGN 30000
static int g_fix_coverity_bat_core;
extern void cap_learning_event_done_notify(void);
extern void dsm_learning_event_report(int fcc);
/* ============================================================ */
/* global variable */
/* ============================================================ */
...
int gauge_reset_ncar(void)
{
	...
	bm_err("gauge_reset_ncar done, %d, version:%d\n",
...
void fgauge_get_profile_id(void)
{
	...
	if (chg_dev )
		bm_debug("Found primary charger [%s]\n",chg_dev->props.alias_name);
	else
		bm_debug("*** Error: can't find primary charger ***\n");



	if ((sizeof(g_battery_id_voltage) / sizeof(g_battery_id_voltage[0])) != BATTERY_ID_NUMBER) {
		bm_debug("[fgauge_get_profile_id]error! voltage range incorrect!\n");
		return;
	}
	if(batt_id_volt_by_adc){
		ret = IMM_GetOneChannelValue_Cali(BATT_ID_CHANNEL_NUM, &id_volt);
		if(ret != 0)
			bm_err("getchannelvalue fail\n");
		bm_err("[fgauge_get_profile_id]am_id_volt = %d\n", id_volt);
		for (id = 0; id < BATT_ID_NUMBER; id++) {
			if (id_volt > g_batt_id_voltage[id].id_voltage_low && id_volt < g_batt_id_voltage[id].id_voltage_high) {
				bm_err("voltag_low:%d  high:%d\n",g_batt_id_voltage[id].id_voltage_low,g_batt_id_voltage[id].id_voltage_high);
				gm.battery_id = id;
				strncpy(battery_vendor_name, g_batt_id_voltage[id].vendor_name, strlen(g_batt_id_voltage[id].vendor_name)+1);
				break;
			}
		}
	}else{
		id_volt = charger_dev_get_ts_vbat_adc(chg_dev);
		bm_err("[fgauge_get_profile_id]id_volt = %d\n", id_volt);
		for (id = 0; id < BATTERY_ID_NUMBER; id++) {
	...
	if(batt_id_volt_by_adc){
		if(id == BATT_ID_NUMBER)
			gm.battery_id = 0;
	}else{
		if (id == BATTERY_ID_NUMBER)
			gm.battery_id = 0;
	}
...
void fg_custom_init_from_dts(struct platform_device *dev)
{
	...
	int ret4;
#endif
	unsigned int vbus_r1 = 0;

	batt_id_volt_by_adc = of_property_read_bool(np, "batt_id_volt_by_adc");
	bm_err("batt_id read:%d\n",batt_id_volt_by_adc);
	fgauge_get_profile_id();
	bat_id = gm.battery_id;

	bm_err("fg_custom_init_from_dts\n");

	if (!of_property_read_u32(np, "rbat_pull_up_r", &vbus_r1))
		gm.rbat.rbat_pull_up_r = vbus_r1;

	if (!of_property_read_u32(np, "vbus_r_charger_1", &vbus_r1)) {
		fg_cust_data.r_charger_1 = vbus_r1;
	}
...
void fg_nafg_monitor(void)
{
	...
	if (gm.last_nafg_cnt != nafg_cnt) {
		...
		if (dtime.tv_sec >= 600) {
			...
			bm_err("[fg_nafg_monitor]after restart fuelgauged\n");
		}
	}
...
void fg_sw_bat_cycle_accu(void)
{
	...
	if (diff_car > 0) {
		bm_err("[fg_sw_bat_cycle_accu]ERROR!drop diff_car\n");
...
void fg_drv_update_hw_status(void)
{
	...
	bm_err("[FGDEBUG]%d %d %d, %d %d %d,%d restar:%d rcv:%d send:%d pid:%d %d\n",
		gm.data_handle_fail,
		gm.kvmalloc_fail,
		gm.skb_fail,
		gm.nlmsg_put_fail,
		...
		gm.netlink_fail,
		gm.force_restart_daemon,
		gm.nl_rcv_data_counts, gm.nl_send_counts,
		gm.g_fgd_pid, gm.old_pid);
...
int battery_update_routine(void *x)
{
	...
	while (1) {
		...
		if (g_fix_coverity_bat_core == EXITLOOP)
			return -1;
	}
}
	...
	char *ret)
{
	...
	bm_trace("fg_daemon_send_data type:%d, tsize:%d size:%d idx:%d\n",
		prcv->type,
		prcv->total_size,
		prcv->size,
		prcv->idx);
...
void fg_daemon_comm_INT_data(char *rcv, char *ret)
{
	...
	case FG_SET_AGING_FACTOR:
		{
			if (first_in < 2) {    // every power on will trigger 2 times, skip
				bm_err("fg_set_aging_factor in %d\n", first_in);
				first_in++;
				break;
			}
			gm.aging_factor = prcv->input;
			gm.fcc = (FCC_MAX_DESIGN * (gm.aging_factor)) / MAH_TO_UAH;
			bm_err("fg_daemon_comm_INT_data gm.aging_factor:%d gm.algo_qmax:%d gm.fcc:%d\n",
			gm.aging_factor,gm.algo_qmax,gm.fcc);
			if((gm.fcc < MIN_DESIGN_UAH) || (gm.fcc > MAX_DESIGN_UAH)){
				bm_err("fg_daemon_comm_INT_data not ok use default fcc \n");
				gm.fcc = MAX_DESIGN_UAH;
			}
...
void bmd_ctrl_cmd_from_user(void *nl_data, struct fgd_nl_msg_t *ret_msg)
{
	...
	case FG_DAEMON_CMD_SET_INIT_FLAG:
		{
			...
			if (gm.init_flag == 1) {
				gm.fcc = FCC_MAX_DESIGN * MAH_TO_UAH;
	...
	break;
	case FG_DAEMON_CMD_IS_AGING_FACTOR_RESET:
	{
		...
		memcpy(ret_msg->fgd_data, &reset, sizeof(reset));
		bm_debug(
			"[fr] FG_DAEMON_CMD_IS_AGING_FACTOR_RESET = %d\n",
			reset);
