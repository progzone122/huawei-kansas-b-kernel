...
#define MTK_INPUT_CURRENT_LIMIT_FOR_JEITA  200000
#define VDPM_MAX    (4600)
#define MV_TO_UV    (1000)

int g_vdpm_first_run = FIRST_RUN_TRUE;
extern void iscd_reset_sample_info(struct iscd_info *iscd);
extern void iscd_main_proc(struct charger_manager *info);
extern void dsm_batt_full_early_report(void);

bool g_dpm_enable = false;
int g_vdpm_buf_limit = VDPM_DELTA_LIMIT_5;
struct charge_vdpm_data g_vdpm_data[VDPM_PARA_LEVEL];
...
void set_vdpm_by_vol(struct charger_manager *info, int vbat)
{
	...
	if ((g_dpm_enable == false) ||
		...
		(info->chr_type == CHARGING_HOST)) {
                ...
                chr_err("%s enalbe:%d, SDP CDP set vbat_dpm=%d ret=%d\n", __func__, g_dpm_enable, vbat_dpm * MV_TO_UV, ret);
		return;
	}

	chr_info("vdpm_first_run = %d\n", g_vdpm_first_run);
	for (i = 0; i < VDPM_PARA_LEVEL; i++) {
		if ((vbat >= g_vdpm_data[i].cap_min) && (vbat < g_vdpm_data[i].cap_max)) {
			if (((last_vbat - vbat <= 0) && (last_i <= i)) ||
				(g_vdpm_data[i].cap_max - vbat > g_vdpm_data[i].cap_back) ||
				(abs(last_i - i) > 1) || (FIRST_RUN_TRUE == g_vdpm_first_run)) {
	...
	g_vdpm_first_run = FIRST_RUN_FALSE;
	last_vbat = vbat;
	last_vdpm = vbat_dpm;
...
extern int huawei_set_charger_input_current;
static void swchg_select_charging_current_limit(struct charger_manager *info)
{
	...
	if (info->enable_sw_jeita) {
		if (IS_ENABLED(CONFIG_USBIF_COMPLIANCE)
			...
			chr_err("pdata->charging_current_limit:%d info->sw_jeita.cur:%d\n",pdata->charging_current_limit,info->sw_jeita.cur);
			pdata->charging_current_limit = min(pdata->charging_current_limit,info->sw_jeita.cur);
			pdata->charging_current_limit = min (pdata->charging_current_limit,info->data.basp_current);
			chr_err("pdata->charging_current_limit:%d info->sw_jeita.cur:%d\n",pdata->charging_current_limit,info->sw_jeita.cur);
		}
	}
	...
	if (info->enable_sw_jeita) {
		if (IS_ENABLED(CONFIG_USBIF_COMPLIANCE)
			...
			pdata->input_current_limit = min(pdata->input_current_limit,MTK_INPUT_CURRENT_LIMIT_FOR_JEITA);
			chr_err("pdata->input_current_limit:%d level=%d\n",pdata->input_current_limit,info->sw_jeita.sm);
		}
	}

	if (huawei_set_charger_input_current != -1 && huawei_set_charger_input_current != 0 && huawei_set_charger_input_current != 1)
	{
		if (huawei_set_charger_input_current < pdata->input_current_limit)
	...
	chr_err("force:%d thermal:%d,%d huawei_input_current:%d pe4:%d,%d,%dsetting:%d %d type:%d usb_unlimited:%d usbif:%d usbsm:%d aicl:%d atm:%d\n",
		_uA_to_mA(pdata->force_charging_current),
		_uA_to_mA(pdata->thermal_input_current_limit),
		_uA_to_mA(pdata->thermal_charging_current_limit),
		_uA_to_mA(huawei_set_charger_input_current),
...
static int mtk_switch_charging_plug_in(struct charger_manager *info)
{
	...
	/* BEGIN: Added for short current:  2018/10/18 */
	//double insurance, if plug out,already reset
	iscd_reset_sample_info(info->iscd);
	/* END:   Added for short current:  2018/10/18 */
	return 0;
}
...
#if 0 /*huawei don't need this function*/
...
static unsigned int g_chgTime = 0;
...
static int mtk_switch_charging_run(struct charger_manager *info)
{
	...
	g_chgTime = swchgalg->total_charging_time;
